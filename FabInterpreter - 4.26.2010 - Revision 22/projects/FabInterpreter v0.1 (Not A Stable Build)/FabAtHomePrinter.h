...
class FabAtHomePrinter
{
     ...
     private:
          ...
          void executePath(Path& path, Bay* bay, const bool flowDuringPath);
     ...
     public:
          ...
          //Returns: "PRINTING", "PAUSED", or "IDLE" depending on what printer is currently doing.
          string state(void);
		...
		//save config filePath
		  static void savePreviousConfigName(string name);

		  //load config filePath
		  static string loadPreviousConfigName();
